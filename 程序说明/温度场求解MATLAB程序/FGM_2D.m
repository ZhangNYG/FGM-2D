...
%      gnode_3th-----------第三边界节点号
    ...
    if nargin < 1
        ...
        for   ie=1:1:element_number                                   %对每个单元循环
...
%        5. 计算单元应力和节点应力
    ...
    for ie=1:1:element_number
     ...
     % 处理温度约束条件，修改整体K矩阵和P向量。采用乘大数法。 
        [bc_number,dummy] = size( gnode_1st ) ;
         for ibc=1:1:bc_number
             n = gnode_1st (ibc, 1 ) ;
             gP(n) = 300* gK(n,n) * 1e12 ;%引入边界温度此处为常量300K      
             gK(n,n) = gK(n,n) * 1e12;
         end
...
%如果判断为第三类边界条件进入下面程序段
    if(m==2)
                ...
                xaverage=(xi+xj+xm)/3;
                yaverage=(yi+yj+ym)/3;
               kconduct=10*exp(xaverage*10+yaverage*10);%c=d=10时
                ai = xj*ym - xm*yj ;
                aj = xm*yi - xi*ym ;
                am = xi*yj - xj*yi ;
                bi = yj - ym ;
                ...
                si=sqrt(bi^2+ci^2);
                %hcon为换热系数
                hcon=10*exp(10*((xj+xm)/2)) %上边界换热系数函数变化h0=10，h=h0exp(cx)
                k(1,1)=fi*(bi^2+ci^2);
...
%      ie  ----- 单元号
        ...
        if (m==2)
                    ...
                    p(1,1)=0;%此处无内热源所以为零，如想添加第一类边界下的内热源，参考书孔26页
                    p(2,1)=0.5*hcon*si*(300*sin(xj*pi/0.02)+300);%此处无内热源所以为零，如想添加第一类边界下的内热源，参考书孔26页
                    p(3,1)=0.5*hcon*si*(300*sin(xm*pi/0.02)+300);%TF=300*sin(xm*pi/20)+300
        end
return
