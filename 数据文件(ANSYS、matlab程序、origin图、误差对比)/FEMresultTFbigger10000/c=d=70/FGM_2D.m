...
%如果判断为第三类边界条件进入下面程序段
    if(m==2)
            ...
            %计算导热系数在这导热系数c=d=0.1，
               ...
               %ansys验证
              % kconduct=1;%热导率
               kconduct=10*exp(xaverage*70+yaverage*70);%c=d=0.1时
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%导热率
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    k=D*exp(cx+dy)
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    D=1
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%     
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%kconduct=exp(xaverage*0.1+yaverage*0.1);
                ...
                si=sqrt(bi^2+ci^2);
                %hcon为换热系数
                hcon=10*exp(70*((xj+xm)/2)) %h%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%上边界换热系数函数变化h0=1，h=h0exp(cx)
                k(1,1)=fi*(bi^2+ci^2);
...
%      ie  ----- 单元号
        ...
        if (m==2)
                    ...
                    hcon=10*exp(70*((xj+xm)/2));%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %hcon为换热系数
                   
                    
                    
                    p(1,1)=0;%此处无内热源所以为零，如想添加第一类边界下的内热源，参考书孔26页
                    p(2,1)=0.5*hcon*si*(10000*sin(xj*pi/0.02)+300);%此处无内热源所以为零，如想添加第一类边界下的内热源，参考书孔26页
                    p(3,1)=0.5*hcon*si*(10000*sin(xm*pi/0.02)+300);%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%TF=300*sin(xm*pi/20)+300
        end
